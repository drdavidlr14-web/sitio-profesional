...
function Page() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-16">
      ...
      <h1 className="mt-2 text-4xl text-[#0b1f38]">{t({ es: 'Docencia', en: 'Teaching' })}</h1>
      <p className="mt-4 max-w-3xl leading-relaxed text-[#5c6b7a]">{t(teaching.intro)}</p>

      <div className="mt-12 space-y-8">
        {teaching.posts.map((p) => (
          <article
            key={p.school}
            className="grid gap-6 overflow-hidden rounded-2xl border border-[#0b1f38]/10 bg-white md:grid-cols-[1.2fr_0.8fr]"
          >
            <div className="p-6 md:p-8">
              ...
              <p className="mt-4 text-sm leading-relaxed text-[#5c6b7a]">{t(p.notes)}</p>
            </div>
            {p.img ? (
      ...
      <section className="mt-16">
        <h2 className="text-2xl text-[#0b1f38]">
          {t({ es: 'Invitaciones y ponencias', en: 'Invitations and talks' })}
        </h2>
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {teaching.talks.map((talk) => (
            <article key={talk.title.es} className="overflow-hidden rounded-2xl border border-[#0b1f38]/10 bg-white">
              {talk.img ? (
                <img src={talk.img} alt={talk.host} className="h-48 w-full object-cover" />
              ) : null}
              <div className="p-5">
