      ...
      <form onSubmit={onSubmit} className="mt-10 space-y-4">
        <label className="block text-sm">
          ...
          <input
            name="name"
            required
            className="mt-1 w-full rounded-xl border border-[#0b1f38]/15 bg-white px-3 py-2 outline-none focus:border-[#c4a46a]"
          />
        </label>
        <label className="block text-sm">
        ...
        <button
          type="submit"
          className="rounded-full bg-[#0b1f38] px-6 py-2 text-xs uppercase tracking-[0.16em] text-white"
        >
          {t({ es: 'Enviar', en: 'Send' })}
        </button>
        {status === 'ok' ? (
          <p className="text-sm text-emerald-800">
            {t({ es: 'Mensaje registrado. Gracias.', en: 'Message recorded. Thank you.' })}
          </p>
        ) : null}
        {status === 'error' ? (
          <p className="text-sm text-red-800">
            {t({ es: 'Revisa los campos e inténtalo de nuevo.', en: 'Please check the fields and try again.' })}
          </p>
        ) : null}
      </form>
    </main>
