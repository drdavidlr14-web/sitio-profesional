...
export function PressList() {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <figure className="card-lift overflow-hidden rounded-2xl border border-[#0b1f38]/10 bg-white">
        <img src={letter.src} alt={t(letter.title)} className="max-h-80 w-full object-cover object-top" />
        <figcaption className="space-y-2 p-5">
          <p className="text-xs uppercase tracking-[0.16em] text-[#c4a46a]">
            {t({ es: 'Documento', en: 'Document' })}
          </p>
      ...
      {press.map((p) => (
        <figure key={p.title.es} className="card-lift overflow-hidden rounded-2xl border border-[#0b1f38]/10 bg-white">
